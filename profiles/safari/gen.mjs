...
${k.hero ? '' : fitLabel(k.label, { family: SANS, weight: 400, font: FONT }).map((l) => `<text x="72" y="${l.y}" font-family="${SANS}" font-size="${l.size}" fill="${W}" stroke="${W}" stroke-width="0.6" text-anchor="middle" filter="url(#lift)">${l.text}</text>`).join('\n')}
